...
use app\admin\model\TaxiCommentModel;

class TaxiComment extends Base
{
    public function index()
    {
        try {
            $key = input('key');
            $community = new TaxiCommentModel();
            if ($key) {
    ...
    {
        try {
            $community = (new TaxiCommentModel())->where('taxi_user_id', $id)->find();
