...
class HotSubject extends Base
{
    public function index()
    {
        try {
            $key = input('key');
            $coupon = Db::name('hot_subject');
            if ($coupon) {
    ...
    public function store()
    {
        ...
        $validate = Loader::validate('HotSubject');
...
//        try {
        $region_id = Db::name('hot_subject')->insert($form);
        if ($region_id) {
...
//        }
    ...
    public function update()
    {
        ...
        if (request()->isAjax()) {
            try {
                ...
                unset($form['file']);
                $form['update_time'] = time();
                $coupon = new HotSubjectModel($form['id']);
        ...
        if (request()->isAjax()) {
            ...
            try {
                $collect_id = Db::name('member_collect')->where('module_id', $id)
                    ->where('module_type', 'hot_subject_model')->delete();
                $id = Db::name('hot_subject')->delete($id);
                Db::commit();
                if ($id) {
    ...
    public function status()
    {
        ...
        Db::startTrans();
        try {
            $status = Db::name('hot_subject')->where(array('id' => $id))->value('status');//判断当前状态情况

            if ($status == 0) {
                $module = Db::name('member_collect')->where('module_id', $id)
                    ->where('module_type', 'hot_subject_model')->update(['delete_time' => time()]);
                $flag = Db::name('hot_subject')->where('id', $id)->update(['status' => 1]);
                Db::commit();
                return json(['code' => 1, 'data' => $flag['data'], 'msg' => '已禁止']);
            } else {
                $module = Db::name('member_collect')->where('module_id', $id)
                    ->where('module_type', 'hot_subject_model')->update(['delete_time' => null]);
                $flag = Db::name('hot_subject')->where('id', $id)->update(['status' => 0]);
                Db::commit();
    ...
    public function detailed_hot($id)
    {
        ...
        $this->assign('id', $id);
        return $this->fetch('hot_subject/detailed');
    }
    ...
    public function add_detailed($id)
    {
        ...
        $this->assign('data', $id);
        // 模板输出
        return $this->fetch('hot_subject/add_detailed');
    }
    ...
    public function store_detailed(\think\Request $request)
    {
        ...
        $file = Db::name('hot_file')->where('community_id', $post['community_id'])->count('id');
        if ($file >= 9) {
        ...
        //执行添加操作
        $res = Db::name('hot_file')->insert($date);
    ...
    public function edit_detailed($id, $community_id)
    {
        ...
        //查询出这条数据(图片)
        $data = Db::name('hot_file')->where('id', $id)->find();

        //加载视图
        $this->assign('data', $data);
        $this->assign('id', $community_id);
        // 模板输出
        return $this->fetch('hot_subject/edit_detailed');
    }
    ...
    {
        //判断有无这个数据
        $res = Db::name('hot_file')->where('id', $id)->find();

        if (!$res) {
            return $arr = ['code' => '2', 'msg' => '删除失败'];
        }
        //执行删除
        $res = Db::name('hot_file')->delete($id);
