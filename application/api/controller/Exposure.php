...
class Exposure
{
    ...
    public function index()
    {
        ...
        try {
        //查询有置顶的动态
        $communitySticky = new ExposureModel();
...
//            //批量更新过期置顶数据
        ...
        $community = new ExposureModel();
        if ($search) {
            $comm = Db::name('exposure')->where('status',0)->where('recommend_status', $recommend_status)->buildSql();
            $commSticky = Db::name('exposure')->where('sticky_status', 0)->union($comm)->buildSql();
            ...
            switch ($order) {
                ...
                default:
                    //最新
                    $comm = Db::name('exposure')->where('status',0)->where('recommend_status', $recommend_status)->buildSql();
                    $commSticky = Db::name('exposure')->alias('com')->where('sticky_status', 0)->union($comm)->buildSql();
        ...
        foreach ($community as $k => $val) {
            ...
            $usedImage = Db::name('exposure_file')->where('exposure_id', 'in', $val['id'])->select();
//                $topic = Db::name('topic_cate')->where('id', 'eq', $val['topic_id'])->select();
            ...
            $praise = Db::name('member_praise')->where('user_id', 'eq', $user_id)
                ...
                ->where('module_type', 'eq', 'exposure')
                ->find();

            //获取收藏数据
            $collect = Db::name('member_collect')->where('user_id', 'eq', $user_id)
                ->where('module_id', 'eq', $val['id'])
                ->where('module_type', 'eq', 'exposure_model')
                ->find();
...
//                $val['region'] = $topic[0];
    ...
    public function save()
    {
        ...
        try {
            ...
            //添加积分
            if($this->addIntegral($id, 'exposure')){
                $explain = '发布成功，积分+5';
            }else{
                $explain = '发布成功';
            }
    ...
    public function show()
    {
        ...
        try {
            $community = ExposureModel::with(['communityFile','user'=>function($query){
    ...
    public function praise()
    {
        ...
        if (!$community_id = input('exposure_id')) {
            throw new BannerMissException([
                'code' => 400,
                'ertips' => '缺少必要参数'
            ]);
        ...
        $explain = '';
        try {
            $community = ExposureModel::get($community_id);
            if (!$community) {
                throw new BannerMissException([
                    'code' => 400,
                    'ertips' => '没有这条数据'
