...
class HotSubject
{
    ...
    public function index()
    {
        ...
        foreach ($community as $k => $val) {
            ...
            $usedImage = Db::name('hot_file')->where('community_id', 'in', $val['id'])->select();
//                $topic = Db::name('topic_cate')->where('id', 'eq', $val['topic_id'])->select();
            ...
            $praise = Db::name('member_praise')->where('user_id', 'eq', $user_id)
                ...
                ->where('module_type', 'eq', 'hot_subject')
                ->find();

            //获取收藏数据
            $collect = Db::name('member_collect')->where('user_id', 'eq', $user_id)
                ->where('module_id', 'eq', $val['id'])
                ->where('module_type', 'eq', 'hot_subject_model')
                ->find();
...
//    }
    ...
    public function show()
    {
        ...
        try {
            $community = HotSubjectModel::with(['communityFile','user'=>function($query){
    ...
    public function praise()
    {
        ...
        $explain = '';
        try {
            $community = HotSubjectModel::get($community_id);
            if (!$community) {
                throw new BannerMissException([
                    'code' => 400,
                    'ertips' => '没有这条数据'
