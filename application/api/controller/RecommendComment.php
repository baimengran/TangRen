...
use app\admin\model\RecommendCommentModel;
...
class RecommendComment
{
    public function index()
    {
        ...
        try {
            $comments = RecommendCommentModel::where('community_id', 'eq', $id)->where('delete_time',0)->order('create_time', 'desc')->paginate(20);
            ...
            foreach ($comments as $comment) {
                ...
                $commentImage = Db::name('recommend_comment_image')->where('comment_id', 'eq', $comment['id'])->select();
                $community = RecommendModel::where('id', 'eq', $comment['community_id'])->find();
    ...
    public function save()
    {
        ...
        $data['user_id'] = $id;
        $validate = validate('RecommendComment');
...
//        return $path;
        try {
            $communityComment = RecommendCommentModel::create([
